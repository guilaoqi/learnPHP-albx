...
<body>
  ...
  <script type="text/javascript">
    $('#logo').on('change',function(){
      ...
      xhr.send(data);
      xhr.onload=function(){
        var path=JSON.parse(xhr.responseText).path.substr(12);
        $('#site_logo').val(path);
        $('#logo').next().attr('src',path);
        console.log(path.substr(12));
      }
    })
    ...
    $('button').on('click',function(){
      var values={
                  ...
                  comment_status:$('#comment_status').prop('checked'),
                  comment_reviewed:$('#comment_reviewed').prop('checked')
                  }

      $.post('./api/edit_setting.php',values,function(res){
        if(res!='0'){
          $('#err').css('display','block').text(res);
        }
        else{
          $('#err').css('display','block').removeClass('alert-danger').addClass('alert-success').text('更改成功');
        }
      })
      return false;
    })
